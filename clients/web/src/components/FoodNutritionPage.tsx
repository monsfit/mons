      ...
      <main className="mx-auto grid max-w-6xl gap-10 px-5 py-10 lg:grid-cols-[minmax(0,1fr)_310px]">
        <div className="min-w-0">
          ...
          <div className="mt-5 flex items-center gap-3">
            ...
            <span className="text-xs text-muted-foreground">Source record {food.source_id}</span>
          </div>
          <label className="mt-8 block text-xs text-muted-foreground">
            Nutrition amount
            <select
              aria-label="Nutrition amount"
              className="mt-2 block w-full rounded-lg border border-border bg-card p-3 text-sm text-foreground"
              value={portionId}
          ...
          <section className="my-8 border-y border-border py-7">
              ...
              {macros.map(([field, label, unit]) => (
                <div key={field}>
                  ...
                  <strong className="flex items-center gap-1 text-lg font-medium">
                    {field === 'calories' && <Flame className="size-4 text-primary" />}
          ...
          <section>
            ...
            {nutrients.length === 0 && (
              <p className="py-6 text-sm text-muted-foreground">No nutrient values reported.</p>
            )}
          </section>
        </div>
        <aside>
          ...
          >
            ...
            <p className="mt-2 text-xs leading-5 text-muted-foreground">Amount: {portion?.label}</p>
            <div className="my-3 flex items-center justify-between border-y-4 border-border py-3">
              <strong>Calories</strong>
              <strong className="text-2xl">{formatNutrient(amount('calories'), '').trim()}</strong>
            </div>
            <p className="mb-2 text-right text-[10px] text-muted-foreground">% Daily Value</p>
            {facts.map(([field, label, unit]) => {
              const value = amount(field)
              const percent = value === null ? null : dailyValuePercent(field, value, unit)
              return (
                <div
                  className="flex justify-between gap-3 border-t border-border py-2 text-xs"
                  key={field}
                >
                  <span>
                    {label}{' '}
